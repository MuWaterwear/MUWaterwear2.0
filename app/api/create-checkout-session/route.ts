...
export async function POST(request: NextRequest) {
  try {
    ...
    if (actualCosts) {
      console.log('Actual costs provided for Lindbergh coupon:', JSON.stringify(actualCosts, null, 2))
      const totalShipping = Object.values(actualCosts).reduce((sum: number, cost: any) => sum + cost.shippingCost, 0)
    ...
    const apparelShippingCost = (qualifiesForFreeShipping || freeShippingCoupon) ? 0 : calculateApparelShipping()
    ...
    const lineItems = items.map((item: any) => {
      ...
      if (couponCode === 'LINDBERGH-LAKE737' && actualCosts && actualCosts[item.id]) {
        ...
        console.log(`Using actual cost for ${item.name}: $${perUnitCost.toFixed(2)} per unit (production + tax)`)
      }
    ...
    const sessionConfig: Stripe.Checkout.SessionCreateParams = {
      ...
      shipping_options: couponCode === 'LINDBERGH-LAKE737' && actualCosts ? [
        {
          // For Lindbergh coupon, use actual shipping costs
          shipping_rate_data: {
            type: 'fixed_amount',
            fixed_amount: {
              amount: Math.round(Object.values(actualCosts).reduce((sum: number, cost: any) => sum + cost.shippingCost, 0) * 100),
              currency: 'usd',
            },
      ...
      ] : hasOnlyApparelAndAccessories ? [
        {
          shipping_rate_data: {
            ...
            display_name: (qualifiesForFreeShipping || freeShippingCoupon) ? 
              (freeShippingCoupon ? 'Free Shipping (Coupon Applied)' : 'Free Shipping') : 
