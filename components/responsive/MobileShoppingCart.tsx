    ...
    <div className={`min-h-screen bg-black text-white ${className}`}>
      ...
      {isMobile && hero && (
        <MobileHero
          ...
          backgroundVideo={hero.video}
          title={hero.title}
          subtitle={hero.subtitle}
        />
      )}
      {/* Main Content */}
      <main className={isMobile ? 'mobile-content' : 'desktop-content'}>{children}</main>
    </div>
  )
}
...
export function MobileHero({
  ...
  return (
    <section
      className={`relative mobile-viewport-height flex flex-col items-center justify-center overflow-hidden ${className}`}
    >
      {/* Background */}
      ...
      <div className="relative z-20 text-center px-6 max-w-lg mx-auto">
        ...
        {subtitle && (
          <p className="text-xl text-gray-300 mb-8 font-light leading-relaxed">{subtitle}</p>
        )}
        {ctaText && onCtaClick && (
          <button
            onClick={onCtaClick}
